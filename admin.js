...
function viewOrder(orderId) {
  ...
  modalBody.innerHTML = `
        <div class="detail-grid">
            ...
            <div class="detail-item">
                <span class="detail-label">Bahasa E-book</span>
                <span class="detail-value">${order.ebook_lang === 'en' ? '🇬🇧 English' : '🇮🇩 Indonesia'}</span>
            </div>
            ${order.verified_at
      ? `
            <div class="detail-item">
      ...
      : ""
            ...
            <div class="detail-item" style="grid-column:1/-1;margin-top:8px;padding-top:16px;border-top:1px solid #e5e7eb;">
                <span class="detail-label" style="font-size:13px;color:#374151;">📎 Bukti Pembayaran</span>
                <div class="proof-section" id="proofSection-${order.order_id}">
                    ${order.payment_proof
      ? `<div class="proof-preview-inline">
                            <img src="${API.proof}?order_id=${encodeURIComponent(order.order_id)}&token=${encodeURIComponent(apiKey)}&t=${Date.now()}" 
                                 alt="Bukti bayar" class="proof-thumb" onclick="openImagePreview(this.src, '${esc(order.order_id)}')">
                            <div class="proof-actions">
                                <button class="btn btn-xs btn-info" onclick="openImagePreview(document.querySelector('#proofSection-${order.order_id} .proof-thumb').src, '${esc(order.order_id)}')">🔍 Preview</button>
                                <button class="btn btn-xs btn-danger" onclick="deleteProof('${order.order_id}')">🗑️ Hapus</button>
                            </div>
                         </div>`
      : `<span style="color:var(--gray-light);font-size:13px;">Belum ada bukti bayar</span>`
    }
                    <div class="proof-upload-area" style="margin-top:12px;">
                        <label class="btn btn-sm btn-primary proof-upload-btn" style="cursor:pointer;">
                            📤 Upload Bukti Bayar
                            <input type="file" accept="image/jpeg,image/png,image/gif,image/webp" 
                                   style="display:none;" onchange="uploadProof('${order.order_id}', this)">
                        </label>
                        <span class="proof-upload-hint">JPG, PNG, GIF, WebP · Maks 5MB</span>
                    </div>
                </div>
            </div>
        </div>
...
// ==================== PAYMENT PROOF ====================
async function uploadProof(orderId, inputEl) {
  ...
  try {
    ...
    if (data.success) {
      showToast("✅ Bukti bayar berhasil di-upload", "success");
      // Update the order in local cache
      const order = allOrders.find((o) => o.order_id === orderId);
      if (order) order.payment_proof = data.filename;
...
async function deleteProof(orderId) {
  if (!confirm("Hapus bukti bayar untuk pesanan ini?")) return;
  ...
  try {
    ...
    if (data.success) {
      showToast("Bukti bayar dihapus", "warning");
      const order = allOrders.find((o) => o.order_id === orderId);
      if (order) order.payment_proof = null;
      viewOrder(orderId);
      loadOrders();
    } else {
      showToast(`Gagal: ${data.message}`, "error");
    ...
    showToast("Gagal menghapus bukti bayar", "error");
  }
}
...
function openImagePreview(src, orderId) {
  ...
  img.src = src;
  if (caption) caption.textContent = `Bukti Bayar — ${orderId}`;
